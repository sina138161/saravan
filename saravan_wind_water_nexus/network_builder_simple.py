...
class WindWaterNetworkBuilder:
    ...
    def build_network(self, turbine_mix: Dict = None,
        ...
        # Add waste-to-energy system
        self._add_waste_to_energy_system()
    ...
    def _add_buses(self):
        ...
        buses = [
            ...
            'biomass',  # Organic waste (biomass)
            'biogas',  # Biogas from anaerobic digestion
            'heat',  # Heat energy
            'compost',  # Compost product
        ]
    ...
    def _add_waste_to_energy_system(self):
        """Add waste-to-energy system: sludge + biomass → biogas/compost → CHP"""

        print(f"\n♻️  Adding waste-to-energy system...")

        # Get water demand for sludge generation estimation
        water_df = self.data['water_demand']
        urban_demand = water_df['urban_m3h'].values[:len(self.network.snapshots)]
        max_urban = urban_demand.max()
        wastewater_factor = 0.80

        # Sludge generation rates (kg dry solids per m³ wastewater treated)
        sludge_rate_primary = 0.05  # kg/m³ (5% of wastewater becomes sludge)
        sludge_rate_secondary = 0.08  # kg/m³ (8% - more biological solids)

        # Convert to energy equivalent for PyPSA (1 kg/h = 0.01 kW equivalent)
        sludge_energy_equiv = 0.01  # kW per kg/h

        # 1. Sludge generation from WW Primary Treatment
        self.network.add(
            "Link",
            "Sludge_Gen_Primary",
            bus0="wastewater_municipal",
            bus1="sludge",
            p_nom=max_urban * wastewater_factor,
            efficiency=sludge_rate_primary * sludge_energy_equiv,
            marginal_cost=0,
            capital_cost=0
        )

        # 2. Sludge generation from WW Secondary Treatment
        self.network.add(
            "Link",
            "Sludge_Gen_Secondary",
            bus0="wastewater_municipal",
            bus1="sludge",
            p_nom=max_urban * wastewater_factor,
            efficiency=sludge_rate_secondary * sludge_energy_equiv,
            marginal_cost=0,
            capital_cost=0
        )

        # 3. Biomass source (agricultural/urban organic waste)
        # Saravan produces ~50 tons/day organic waste
        biomass_available = 50 * 1000 / 24  # kg/h
        biomass_p_nom = biomass_available * sludge_energy_equiv  # kW equivalent
        ...
        self.network.add(
            ...
            bus="biomass",
            p_nom=biomass_p_nom,
            p_max_pu=1.0,  # Constant availability
            p_min_pu=0,
            marginal_cost=2,  # Small cost for collection/transport
            capital_cost=0,  # Waste is free
            carrier='biomass'
        )

        print(f"\n   Biomass availability: {biomass_available:.0f} kg/h ({biomass_available*24/1000:.1f} tons/day)")

        # 4. Anaerobic Digester (sludge + biomass → biogas)
        # Reference: Metcalf & Eddy (2014) - biogas yield ~0.5 m³/kg VS
        biogas_yield = 0.5  # m³ biogas per kg volatile solids
        ch4_content = 0.65  # 65% CH4 in biogas
        ch4_energy = 10  # kWh/m³ CH4 (Lower Heating Value)

        digester_electric = 0.15  # kWh electricity per kg sludge (mixing, pumping)
        digester_heat = 0.25  # kWh heat per kg sludge (maintain 35°C mesophilic)

        # Biogas energy output per kg input
        biogas_energy_per_kg = biogas_yield * ch4_content * ch4_energy
        # kW biogas per kW sludge+biomass input
        biogas_efficiency = biogas_energy_per_kg / (1 / sludge_energy_equiv)

        max_digester_capacity = (max_urban * wastewater_factor *
                                 (sludge_rate_primary + sludge_rate_secondary) +
                                 biomass_available)
        ...
        self.network.add(
            ...
            bus0="sludge",
            bus1="electricity",  # Electricity consumed
            bus2="heat",  # Heat consumed
            bus3="biogas",  # Biogas produced
            p_nom=max_digester_capacity * sludge_energy_equiv,
            efficiency=digester_electric * sludge_energy_equiv,  # Electricity per kg
            efficiency2=digester_heat * sludge_energy_equiv,  # Heat per kg
            efficiency3=biogas_efficiency,  # Biogas output
            marginal_cost=1,  # Operating cost
            capital_cost=300000  # Digester tank + equipment
        )

        # Also consume biomass in digester
        self.network.add(
            "Link",
            "Biomass_to_Digester",
            bus0="biomass",
            bus1="sludge",  # Mix with sludge
            p_nom=biomass_p_nom,
            efficiency=1.0,  # Direct conversion
            marginal_cost=0,
            capital_cost=0
        )

        print(f"\n   Anaerobic Digester:")
        print(f"      Capacity: {max_digester_capacity:.0f} kg/h")
        print(f"      Biogas yield: {biogas_yield} m³/kg VS")
        print(f"      CH4 content: {ch4_content*100}%")
        print(f"      Energy: {digester_electric} kWh/kg elec, {digester_heat} kWh/kg heat")
        print(f"      Biogas energy: {biogas_energy_per_kg:.1f} kWh/kg")
        print(f"      CAPEX: $300,000")

        # 5. Composting (sludge + biomass → compost fertilizer)
        # Reference: EPA composting guidelines
        compost_yield = 0.40  # 40% of input becomes sellable compost (rest moisture/CO2)
        compost_price = 50  # $/ton compost (market price)
        compost_energy = 0.08  # kWh electricity per kg (aeration, turning)

        # Convert price to marginal benefit (negative cost)
        compost_revenue_per_kg = (compost_yield * compost_price / 1000)  # $/kg
        ...
        self.network.add(
            ...
            bus0="sludge",
            bus1="electricity",  # Electricity consumed
            bus2="compost",  # Compost produced
            p_nom=max_digester_capacity * sludge_energy_equiv,
            efficiency=compost_energy * sludge_energy_equiv,
            efficiency2=compost_yield,
            marginal_cost=-compost_revenue_per_kg,  # Negative = revenue!
            capital_cost=150000  # Composting facility
        )

        # Biomass to composting
        self.network.add(
            "Link",
            "Biomass_to_Composting",
            bus0="biomass",
            bus1="sludge",  # Mix with sludge for composting
            p_nom=biomass_p_nom,
            efficiency=1.0,
            marginal_cost=0,
            capital_cost=0
        ...
        self.network.add(
            ...
            "Compost_Sales",
            bus="compost",
            e_nom=1e6,  # Unlimited market
            e_cyclic=False,
            e_initial=0
        )

        print(f"\n   Composting:")
        print(f"      Compost yield: {compost_yield*100}%")
        print(f"      Market price: ${compost_price}/ton")
        print(f"      Revenue: ${compost_revenue_per_kg*1000:.2f}/ton")
        print(f"      Energy: {compost_energy} kWh/kg")
        print(f"      CAPEX: $150,000")

        # 6. CHP (Combined Heat and Power) - burns biogas
        # Reference: EPA CHP Partnership - biogas CHP efficiency
        chp_elec_efficiency = 0.35  # 35% electrical efficiency
        chp_heat_efficiency = 0.45  # 45% heat recovery
        chp_total_efficiency = chp_elec_efficiency + chp_heat_efficiency  # 80% total

        # Biogas heating value already in kWh from digester
        max_biogas = max_digester_capacity * biogas_energy_per_kg

        self.network.add(
            "Link",
            "CHP_Biogas",
            bus0="biogas",
            bus1="electricity",  # Electricity output
            bus2="heat",  # Heat output
            p_nom=max_biogas,
            efficiency=chp_elec_efficiency,  # 35% to electricity
            efficiency2=chp_heat_efficiency,  # 45% to heat
            marginal_cost=5,  # O&M cost
            capital_cost=500000  # CHP engine + generator
        )

        print(f"\n   CHP (Combined Heat & Power):")
        print(f"      Fuel: Biogas (CH4)")
        print(f"      Electrical efficiency: {chp_elec_efficiency*100}%")
        print(f"      Heat efficiency: {chp_heat_efficiency*100}%")
        print(f"      Total efficiency: {chp_total_efficiency*100}%")
        print(f"      Capacity: {max_biogas:.0f} kW (biogas input)")
        print(f"      Elec output: {max_biogas * chp_elec_efficiency:.0f} kW")
        print(f"      Heat output: {max_biogas * chp_heat_efficiency:.0f} kW")
        print(f"      CAPEX: $500,000")

        # 7. Heat storage (optional - thermal storage tank)
        heat_storage_capacity = 1000  # kWh thermal

        self.network.add(
            "Store",
            "Heat_Storage",
            bus="heat",
            e_nom=heat_storage_capacity,
            e_cyclic=False,
            e_initial=heat_storage_capacity * 0.3,  # 30% initial
            standing_losses=0.05,  # 5% per hour heat loss (poor insulation)
            capital_cost=20000  # Insulated water tank
        )

        print(f"\n   Heat Storage:")
        print(f"      Capacity: {heat_storage_capacity} kWh thermal")
        print(f"      Heat losses: 5%/hour")
        print(f"      CAPEX: $20,000")

        # 8. CO2 emissions tracking
        # Anaerobic digestion: ~0.1 kg CO2/kg input (methane leakage)
        # Composting: ~0.05 kg CO2/kg input (aerobic decomposition)
        # CHP combustion: ~0.25 kg CO2/kWh biogas (but carbon-neutral from biomass)

        print(f"\n   CO2 Emissions (lifecycle):")
        print(f"      Anaerobic Digester: 0.10 kg CO2/kg input (methane leakage)")
        print(f"      Composting: 0.05 kg CO2/kg input (decomposition)")
        print(f"      CHP: 0.25 kg CO2/kWh biogas (carbon-neutral from biomass)")
        print(f"      Net impact: Carbon-neutral (biogenic CO2)")
    ...
    def _add_loads(self):
        ...
        # Heat demand (for waste-to-energy CHP system)
        # Urban heating demand for residential space heating
        # Saravan has cold winters (Dec-Feb), heating needed when T < 18°C
        if 'temperature' in self.data:
            temperatures = self.data['temperature']['temperature_c'].values[:len(self.network.snapshots)]

            # Simple heating demand model: proportional to temperature deficit
            # Q_heat = k * (T_comfort - T_outdoor) when T_outdoor < T_comfort
            # Assumes 200 households × 50 m² × 0.1 kW/m² heating capacity
            heating_capacity_kw = 200 * 50 * 0.1  # 1000 kW peak
            comfort_temp = 18  # °C

            # Calculate heating demand (only when cold)
            temp_deficit = np.maximum(0, comfort_temp - temperatures)
            urban_heat_demand = temp_deficit * (heating_capacity_kw / 15)  # Scale by max deficit (~15°C)

            self.network.add(
                "Load",
                "Urban_Heat_Demand",
                bus="heat",
                p_set=urban_heat_demand
            )

            print(f"\n   Heat demand:")
            print(f"      Urban heating: {urban_heat_demand.mean():.1f} kW (avg), {urban_heat_demand.sum():,.0f} kWh (total)")
            print(f"      Peak demand: {urban_heat_demand.max():.1f} kW")
            print(f"      Digester heating: Handled by Anaerobic_Digester Link (0.25 kWh/kg)")
            print(f"      NOTE: CHP heat output serves both urban and digester needs")
        else:
            # Fallback: assume moderate heating demand
            urban_heat_demand = np.ones(len(self.network.snapshots)) * 200  # 200 kW baseload
            self.network.add(
                "Load",
                "Urban_Heat_Demand",
                bus="heat",
            ...
            print(f"\n   Heat demand:")
            print(f"      Urban heating: 200 kW baseload (no temperature data)")
