...
class GasMicroturbine(TechnologyBase):
    ...
    def calculate_outputs(self, fuel_input_kwh: float,
        ...
        Where:
            ...
            q_gt(t): NG/biogas consumption (m³)
            LHV_ch4: Lower heating value of fuel (kWh/m³)

        Args:
            fuel_input_kwh: Fuel energy input (kWh thermal) = q_gt(t) × LHV_ch4
        ...
        # Find efficiency η_gt
        if load_fraction in eff_curve:
            eta_gt = eff_curve[load_fraction]
        else:
            ...
            for i in range(len(loads) - 1):
                if loads[i] <= load_fraction <= loads[i+1]:
                    ...
                    eta_gt = y0 + (y1 - y0) * (load_fraction - x0) / (x1 - x0)
                    break
            else:
                eta_gt = self.specs['electrical_efficiency']

        # Calculate electricity output using exact formula
        # p_gt(t) = η_gt × q_gt(t) × LHV_ch4
        # where fuel_input_kwh = q_gt(t) × LHV_ch4
        p_gt = eta_gt * fuel_input_kwh
        electricity_output = p_gt
        ...
        return {
            ...
            'electrical_efficiency': eta_gt,
            'eta_gt': eta_gt,  # Explicitly include η_gt for heat recovery formula
            'electricity_kwh': electricity_output,
            'p_gt': p_gt,  # Power generated by GT
    ...
    def calculate_fuel_requirement(self, electricity_demand_kwh: float,
        ...
        # From p_gt(t) = η_gt × q_gt(t) × LHV_ch4
        # fuel_input_kwh = q_gt(t) × LHV_ch4 = p_gt(t) / η_gt
        fuel_input_kwh = electricity_demand_kwh / eta_gt
